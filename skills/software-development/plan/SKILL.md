...
### Missing Project Convention Validation

**Before writing the plan, validate your design decisions against the project's own docs and existing implementations.** The user will catch misplaced categories, wrong constant locations, or non-idiomatic patterns — avoid the rework by checking first.

**Bad:** Guess a new layer's category, guess the filter pattern, guess the constant location. User corrects 3+ times.
**Good:** Before finalizing, read:

1. `AGENTS.md` / `CONTRIBUTING.md` / project root conventions — naming, file structure, architecture rules.
2. **Existing implementations** — search for similar features already in the codebase. Mirror their file locations, registry entries, and patterns exactly.
3. **Project docs** — check `docs/` for dedicated how-to guides (map layers, chart patterns, widget layout, etc.).
4. **The existing `layerQuery` initial state** — match the pattern of how other layers define their query defaults (store constants vs hardcoded).
5. **Category/registry placement** — verify which category array a new layer belongs to by checking where similar layers live.

The plan skill says to "explore the codebase" — make sure this exploration specifically covers the project's own architectural docs and existing implementations of the same class of feature before you commit to a design.

### Unvalidated Assumption About Feature Groups
